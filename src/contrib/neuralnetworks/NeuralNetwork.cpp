...
NeuralNetwork::NeuralNetwork(unsigned int nbInputs, unsigned int nbOutputs) :
		_weights(*(new std::vector<double>(0))), _nbInputs(nbInputs), _nbOutputs(nbOutputs), _requiredNumberOfWeights(0),
		_haveInputsChanged(false), _haveOutputsChanged(false), _haveWeightsChanged(false) {
	_inputs = std::vector<double>(_nbInputs, 0.0);
	_outputs = std::vector<double>(_nbOutputs, 0.0);
}
