...
for filename in filenames:
	...
	for j in range(len(fileContentArray)):
		...
		if(match):
			if(not map.has_key(match.group(1))):
				map[match.group(idEvEvaluation)] = {}
			map[match.group(idEvEvaluation)]['activeCount'] = match.group(2)
	...
	for iteration in range(int(options.begin),maxIteration+1):
		...
		if map.has_key(str(iteration)):
			...
			if(map[str(iteration)].has_key('activeCount')):
				activationCount = int(map[str(iteration)]['activeCount'])
				somethingToPrint = 1
			...
			if ( somethingToPrint == 1 ):
				...
				fileData.write(str(activationCount))
				fileData.write("\n")
	...
	logPlotContent = 'plot '
	cpt = 2
	logPlotContent=logPlotContent+' \'tmp.dat\' using 1:2 notitle with line,'
	logPlotContent = logPlotContent[:-1]
	
	#TODO : the title is in logplothead. Change it so it match the infos in debugXX.properties
	copyfile('logplotheadAQ',filePath + '/logplot.gp')
	fileGnuplot = open(filePath + '/logplot.gp',"a")
	fileGnuplot.write('set output \'' + re.sub('\.log','AQ.eps',filename) + '\'\n')
