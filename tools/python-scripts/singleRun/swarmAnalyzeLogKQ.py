...
for filename in filenames:
	...
	for j in range(len(fileContentArray)):
		patternDescription = "Info\(([0-9]+)\) : ([0-9]+)\(.*\) get an energy point at .* :: Value : ([0-9]+\.[0-9]+)"
		patternEvaluation = re.compile(patternDescription)
		match = patternEvaluation.match(fileContentArray[j])
		if(match):
			if(not map.has_key(match.group(idEnEvaluation))):
				map[match.group(idEnEvaluation)] = {}
			if(not map[match.group(idEnEvaluation)].has_key(match.group(idEnRobot))):
				map[match.group(idEnEvaluation)][match.group(idEnRobot)] = {}
			map[match.group(idEnEvaluation)][match.group(idEnRobot)]['energyLoaded'] = match.group(idEnValue)
	...
	for iteration in range(int(options.begin),maxIteration+1):
		...
		if map.has_key(str(iteration)):
			...
			for robot in map[str(iteration)]:
				if(map[str(iteration)][robot].has_key('energyLoaded')):
					energyList.append(float(map[str(iteration)][robot]['energyLoaded']))
					updateEnergy = True
				if(map[str(iteration)][robot].has_key('HumanEnergy')):
					humanEnergyCpt = humanEnergyCpt + 1
					lastRestart = iteration

		if ( ( float(iteration+1)/100.0 ) % float(options.step) == 0 ):
			if updateEnergy:
				energyQuartile = computeQuartiles(energyList)
				energyList = []
			else:
				energyQuartile = [0,0,0,0,0]
			...
			fileData.write(","+ str(energyQuartile[0]) +","+ str(energyQuartile[1]) +","+ str(energyQuartile[2]) +","+ str(energyQuartile[3]) +","+ str(energyQuartile[4]) )#+ "," + str(humanEnergyCpt) )
			fileData.write("\n")
		else:
			fileData.write(str(iteration/100)) 
	...
	logPlotContent = 'plot '
	cpt = 2
	logPlotContent='plot \'tmp.dat\' using 1:' + str(cpt+1) + ':'+ str(cpt) + ':' + str(cpt+4) + ':' + str(cpt+3) +' with candlesticks lt 3 lw 2 title \'Quartiles of the energy earned by the robots\' whiskerbars, \'\' using 1:' + str(cpt+2) + ':'+ str(cpt+2) + ':' + str(cpt+2) +':' + str(cpt+2) + ' with candlesticks lt -1 lw 2 notitle,'
	cpt += 5
	...
	copyfile('logplotheadKQ',filePath + '/logplot.gp')
	fileGnuplot = open(filePath + '/logplot.gp',"a")
	fileGnuplot.write('set output \'' + re.sub('\.log','KQ.eps',filename) + '\'\n')
