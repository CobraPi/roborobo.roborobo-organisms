...
for filename in filenames:
	...
	for iteration in range(int(options.begin),maxIteration+1):
		...
		if map.has_key(str(iteration)):
			...
			if ( ( float(iteration+1)/400.0 ) % float(options.step) == 0 ):
				sigmaQuartille = computeQuartiles(sigmaList)
				fileData.write(str(iteration/400)) 
				fileData.write(","+ str(sigmaQuartille[0]) +","+ str(sigmaQuartille[1]) +","+ str(sigmaQuartille[2]) +","+ str(sigmaQuartille[3]) +","+ str(sigmaQuartille[4]) ) #+ "," + str(humanEnergyCpt) )
				sigmaList=[]
				fileData.write("\n")
			#else:
	...
	logPlotContent = 'plot '
	cpt = 2
	logPlotContent='plot \'tmp.dat\' using 1:' + str(cpt+1) + ':'+ str(cpt) + ':' + str(cpt+4) + ':' + str(cpt+3) +' with candlesticks lt 3 lw 2 title \'Quartiles of the average value of sigma\' whiskerbars, \'\' using 1:' + str(cpt+2) + ':'+ str(cpt+2) + ':' + str(cpt+2) +':' + str(cpt+2) + ' with candlesticks lt -1 lw 2 notitle,'
	cpt += 5
	...
	copyfile('logplotheadSQ',filePath + '/logplot.gp')
	fileGnuplot = open(filePath + '/logplot.gp',"a")
	fileGnuplot.write('set output \'' + re.sub('\.log','SQ.eps',filename) + '\'\n')
