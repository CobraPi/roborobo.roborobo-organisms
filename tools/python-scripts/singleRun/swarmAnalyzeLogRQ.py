...
parser.add_option("-s","--step", dest="step",help="", default="1" )
...
for filename in filenames:
	...
	for iteration in range(int(options.begin),maxIteration+1):
		...
		if map.has_key(str(iteration)):
			...
			if ( ( float(iteration+1)/400.0 ) % float(options.step) == 0 ):
				if ( not iterationPrinted == iteration ):
					fileData.write(str(float(iteration+1)/400.0) + "," + str(humanEnergyCpt) + "\n")
					humanEnergyCpt = 0
					iterationPrinted = iteration
	...
	logPlotContent = 'plot '
	cpt = 2
#	logPlotContent='plot \'tmp.dat\' using 1:' + str(cpt+1) + ':'+ str(cpt) + ':' + str(cpt+4) + ':' + str(cpt+3) +' with candlesticks lt 3 lw 2 title \'Quartiles of the average value of deltaEnergy\' whiskerbars, \'\' using 1:' + str(cpt+2) + ':'+ str(cpt+2) + ':' + str(cpt+2) +':' + str(cpt+2) + ' with candlesticks lt -1 lw 2 notitle,'
#	cpt += 5
	logPlotContent=logPlotContent+' \'tmp.dat\' using 1:($' + str(cpt) + ' != 0 ? $'+ str(cpt)+' : 0) title \"nb of revived robots\" with impulses linetype 1,'
	logPlotContent = logPlotContent[:-1]
	
	#TODO : the title is in logplothead. Change it so it match the infos in debugXX.properties
	copyfile('logplotheadRQ',filePath + '/logplot.gp')
	fileGnuplot = open(filePath + '/logplot.gp',"a")
	fileGnuplot.write('set output \'' + re.sub('\.log','RQ.eps',filename) + '\'\n')
